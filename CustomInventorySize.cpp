...
public:
	...
	void setup() override {
		...
		::subscribe<&AFGPlayerController::BeginPlay>(std::bind(&CustomInventorySize::beginPlay, this, _1, _2)); //bind the beginPlay function, with placeholder variables
		...
		::subscribe<&PlayerInput::InputKey>([this](Functions::ModReturns* modReturns, PlayerInput* playerInput, FKey key, InputEvent event, float amount, bool gamePad) {
			...
			if (GetAsyncKeyState('K')) {
				...
				If i used this:
					SDK::UFGInventoryComponent inventory = *aCharachter.GetInventory();
				I could extract the inventory size like below but nstead of the -> I used a .
				But once running the resize function i got access violation exception

				From my knowledge:
					placing a * after the class type means it's a reference to a memory adress containing that class
					placing a * before aCharachter.GetInventory() means i get the object and not he reference

					What I'm confused about is the difference between the two thing i done and why it works now and not before...
