	...
	void setup() override {
		...
		::subscribe<&PlayerInput::InputKey>([this](Functions::ModReturns* modReturns, PlayerInput* playerInput, FKey key, InputEvent event, float amount, bool gamePad) {
			...
			if (GetAsyncKeyState('K')) {
				...
				//SDK::UFGHealthComponent health = *aCharachter.GetHealthComponent();
				//float amount = health.GetCurrentHealth();
				//LOG("Health size: ");
				//LOG(amount);
				//LOG("mAjusted");
				//LOG(inventory.mAdjustedSizeDiff);

				//int newsize = size + amount/10 - inventory.mAdjustedSizeDiff;
				int newsize = size+1;
				LOG(newsize);
				LOG("To:");
				inventory2->Resize(newsize);
			}
